...
def broadcast(evends):
	...
	global mycursor
	mycursor.execute("SELECT subscriber, first_name, last_name FROM subscriber")
	...
	for subscriber in myresult:
		first_name = subscriber[1] if type(subscriber[1]) is str else 'halo'
		last_name = subscriber[2] if type(subscriber[2]) is str else 'gaes'
		requests.get('https://api.telegram.org/bot'+bot_token+'/sendMessage?chat_id='+subscriber[0]+'&text=Halo '+first_name+' '+last_name+',\nUpdate SIPRESMAWA '+datetime.now().strftime("%d/%m/%Y %H:%M:%S"))
		for evend in evends:
